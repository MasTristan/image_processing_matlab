...
%segmentation a partir de l'image dilatée, 1ere étape
segmented=dilat;
etiquette=0;
for i=2:taillex
    for j=2:tailley
...
figure(10)
imagesc(segmented),title('1ere etape de segmentation'),colorbar;colormap('jet');
...
figure(11)
imagesc(segmented_final),title('2e etape de segmentation'),colorbar;colormap('jet');

%3eme étape de segmentation, comme l'étape précedente mais le balayage est cette fois ci du haut vers le bas et toujours de droite a  gauche
for i=2:taillex
    for j=2:tailley
        j=tailley-j+1;
            if (dilat(i,j)==255)
...
for i=2:taillex
    for j=2:tailley
 if segmented_final(i,j)==1 || segmented_final(i,j)==2
                segmented_final(i,j)=100;
 end
    end
end
...
figure(12)
imagesc(segmented_final),title('Etape finale de la segmentation'),colorbar;
...
sumi=0;sumj=0;effectif=0;
 ...
 text(coord(2),coord(1), ['objet n° ',num2str(m)])
 end
